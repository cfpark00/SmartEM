...
def test_get_microscope():
    ...
    assert isinstance(microscope, FakeRandomMicroscope), "Returned object is not a FakeRandomMicroscope"
    expected_params = {"W": 1024, "H": 1024, "dtype": np.uint16}
    assert microscope.params == expected_params, "Microscope parameters do not match expected for 'fake' type"
    ...
    assert isinstance(microscope, FakeDataMicroscope), "Returned object is not a FakeDataMicroscope"

...
def test_image_processing_functions():
    ...
    assert np.array_equal(
        int_image, np.array([[127, 255], [0, 63]], dtype=np.uint8)
    ), "float_to_int conversion failed"
    ...
    assert np.allclose(
        float_image,
        np.array([[100 / 255, 200 / 255], [0 / 255, 1.0]], dtype=np.float32),
    ), "int_to_float conversion failed"
    ...
    assert np.all(prob >= 0) and np.all(
        prob <= 1
    ), "Probabilities are not within the range [0, 1]"
    assert (
        prob.shape == test_img.shape
    ), "Output probability map shape does not match input image shape"

...
def test_smart_em_operations_using_fake_data_and_microscope():
    ...
    assert rescan_map.dtype == bool, f"rescan_map is not of type bool, rather of type {rescan_map.dtype} "
    ...
    expected_data = [
        ...
        (None, "merged_em - fast_em")  # Difference data is processed within the function
    ]

    # Check the titles and contents of each subplot
    for ax, (expected_array, expected_title) in zip(fig.axes, expected_data):
        assert ax.get_title() == expected_title, f"Expected title '{expected_title}' but got '{ax.get_title()}'"
        displayed_image = ax.get_images()[0].get_array().data
        if expected_array is not None:
            np.testing.assert_array_almost_equal(displayed_image, expected_array, decimal=5,

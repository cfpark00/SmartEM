...
def test_dataset_getitem(setup_dataset):
    ...
    for i in range(len(dataset)):
        ...
        frame_file, mask_file = dataset.get_file_path(i)
        ...
        assert (
            frame_tensor.dtype == torch.float32
        ), "Frame tensor should be of type torch.float32"
        assert (
            mask_tensor.dtype == torch.int64
        ), "Mask tensor should be of type torch.int64"
        assert (
            frame_tensor.shape[0] == 1

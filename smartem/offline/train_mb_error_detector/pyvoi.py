...
def VI(
    ...
    This is slightly different than the implementation of VI at ./smartem/offline/train_mb_error_detector/tools.py
...
def VI_torch(labels1, labels2, device="cpu", return_split_merge=False):
    ...
    sm_unique, sm_inverse, sm_counts = torch.unique(
        labels2, sorted=False, return_inverse=True, return_counts=True
    )
    fm_unique, fm_inverse, fm_counts = torch.unique(
        labels1, sorted=False, return_inverse=True, return_counts=True
    )
    _, mutual_inverse, mutual_counts = torch.unique(
        mutual_labels, sorted=False, return_inverse=True, return_counts=True
    )
